...
% CEUR-WS format — Short/Position Paper (5 pages)
...
\author{Johannes Kuhn}[%
  email=johannes.kuhn@perelyn.com,
]
\author{Max Becker}[%
  email=slawomir.garcarz@perelyn.com,
]
\author{Sebastian Fetz}[%
  email=max.becker@perelyn.com,
]
...
\begin{abstract}
Manufacturing faces an unprecedented knowledge crisis: 70\% of critical
operational expertise is undocumented and resides in aging workers'
heads. Current AI-based knowledge capture treats this as a
\emph{documentation problem}---extracting expertise into static SOPs,
knowledge graphs, or video tutorials. We argue it should be reframed as
an \emph{agent memory problem}. Drawing on a landscape analysis of 17+
commercial platforms and the agentic memory literature, we identify a
critical \emph{episodic-temporal gap}: no existing system captures the
situated, evolving, episode-grounded nature of expert knowledge. We
propose a three-stage framework---\textsc{Capture},
\textsc{Organize}, \textsc{Transfer}---in which agentic AI orchestrates
multimodal, minimally invasive knowledge ingestion, structures it into
temporally versioned memory stores, and delivers it contextually to new
workers as persistent \emph{knowledge vessels} that absorb, evolve, and
transfer expertise across workforce generations.
\end{abstract}
...
troubleshooting knowledge~\cite{dovient2025}. \emph{Fortune} warned in
January 2026 that ``there are an estimated 1--2 years left to capture
decades of industrial knowledge''~\cite{fortune2026}.

An estimated 70\% of critical manufacturing knowledge is never
...
ramp-up extends from 3--4 months to
8--12~months~\cite{dovient2025}.

A rapidly growing ecosystem of commercial platforms and academic
prototypes has emerged (Section~\ref{sec:landscape}). Yet virtually all
treat tacit knowledge preservation as a \emph{documentation problem}:
conduct interviews, record SOPs, transcribe video---and store results as
static artifacts. This framing discards the very qualities that make
expert knowledge valuable: its situatedness, temporal evolution, and
episode-grounded nature.

We propose reframing tacit knowledge preservation as an \emph{agent
memory problem}. Recent surveys reveal that all major agent harnesses
lack episodic memory and temporal
versioning~\cite{banf2026memory}---precisely the qualities that make
expert knowledge valuable. We present a three-stage framework
(\textsc{Capture}--\textsc{Organize}--\textsc{Transfer}) in which
agentic AI systems serve as persistent \emph{knowledge vessels} that
incrementally absorb, structure, and deliver expertise long after the
original expert has retired.
...
context graph, IBM Maximo~9.1 with watsonx.ai).

\paragraph{Academic prototypes.}
On the research side, Kernan Freire et al.\ deployed an LLM-based
...
session at SME-friendly API costs~\cite{radhakrishnan2025}.

\paragraph{Funded initiatives.}
The EU Horizon project COGNIMAN develops cognitive digital twins
...
\paragraph{The common blind spot.}
...
knowledge \emph{while experts work}, combining multiple modalities:

We identify an \emph{invasiveness spectrum} ranging from fully passive
to lightly interactive, each targeting different knowledge layers:

\begin{itemize}
\item \textbf{Passive process mining (zero worker effort).}
...
\item \textbf{Ambient observation (no behavior change).} Computer
...
\item \textbf{Agentic micro-interviews (brief, contextual).} Rather
...
\item \textbf{Retroactive synthesis.} The system continuously ingests
...
\subsection{Illustrative Walkthrough: Bearing Failure After Material
  Substitution}\label{sec:walkthrough}

To ground the framework concretely, we trace a representative
manufacturing scenario through all three stages.

\paragraph{Context.} A hydraulic press (Press\,7) uses polymer seals
...
This walkthrough illustrates what no current system provides: the
...
\paragraph{Polanyi's paradox and the limits of capture.}
...
but cannot fully replicate the experience. The framework must be
transparent about this boundary.

\paragraph{The apprenticeship dilemma.}
...
transmitted. Our framework addresses this by positioning the agent as a
\emph{complement} to human mentoring---consistent with the empirical
finding that workers prefer human experts when
available~\cite{freire2024}.

\paragraph{Validation, security, and regulation.}
...
95\%~\cite{owasp2026}. Concurrently, the EU AI Act (August 2026)
classifies AI in safety-critical manufacturing as high-risk, mandating
transparency, human oversight, and tamper-evident audit trails. The
framework requires validation mechanisms including expert review
workflows and cross-referencing against sensor data.

\paragraph{SME accessibility.}
...
effective capture need not require heavy IT infrastructure.
...
that make expert knowledge most valuable. We propose reframing tacit
knowledge preservation as an agent memory problem and present the
\textsc{Capture--Organize--Transfer} framework, in which agentic AI
systems serve as persistent \emph{knowledge vessels}---absorbing
expertise while experts work, structuring it with episodic--temporal
grounding, and transferring it contextually to complement human
mentoring. The gap is actionable: the agentic AI infrastructure and the
manufacturing knowledge management ecosystem both exist in mature
form---but have not been connected through an episodic-temporal memory
architecture. Bridging this gap is both a research opportunity and an
...
\bibitem{apqc2025} APQC: Knowledge exodus from the silver tsunami in
  the industrial products sector. Report (2025)
...
\bibitem{cogniman2025} COGNIMAN Consortium: Cognitive digital twin for
  human-robot collaborative manufacturing. J.\ Intell.\ Manuf.\ (2025)
...
\bibitem{explainableml2024} Sch\"ott, P., et al.: Automatic generation
  of insights from workers' actions in manufacturing.
  arXiv:2406.12732 (2024)
...
\bibitem{najjar2025} Najjar, R.: From AI-based KM assistant to tacit
  knowledge agent. 3R Knowledge Services (2025)
